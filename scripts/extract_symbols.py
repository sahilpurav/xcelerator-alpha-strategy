...
def clean_and_extract_symbols(text):
    ...
    for line in lines:
        ...
        if any(
            skip_word in cleaned_line.upper()
            for skip_word in [
                "NIFTY",
                "INDEX",
                "PAGE",
        ...
        concat_match3 = re.match(
            r"^([A-Z0-9&-]{3,15})([^A-Z0-9].*|[A-Z][a-z])", cleaned_line
        )
        ...
        if concatenated_match:
            ...
            if "POWERINDIA" in potential_symbol:
                print(
                    f"DEBUG: Found POWERINDIA case: '{potential_symbol}' + '{remaining_text}'"
                )
            ...
            if potential_symbol in concatenation_fixes:
                print(
                    f"DEBUG: Applying concatenation fix: {potential_symbol} -> {concatenation_fixes[potential_symbol]}"
                )
            ...
            if not was_concatenation_fixed:
                for pattern in company_patterns:
                    if re.match(pattern, remaining_text):
                        print(
                            f"DEBUG: Found company pattern '{pattern}' in remaining text: '{remaining_text}' for symbol: '{potential_symbol}'"
                        )
                        ...
                        for ending in known_endings:
                            if potential_symbol.endswith(ending) and len(
                                potential_symbol
                            ) > len(ending):
                                ...
                                if len(trimmed_symbol) >= 3:
                                    print(
                                        f"DEBUG: Trimming '{ending}' from '{potential_symbol}' -> '{trimmed_symbol}'"
                                    )
            ...
            is_repetitive = any(
                part * 2 in potential_symbol
                for part in ["ICICI", "HDFC", "BAJAJ", "TATA", "BIRLA"]
            )
            has_company_suffix = any(
                potential_symbol.endswith(suffix)
                for suffix in ["LTD", "LIMITED", "CORP"]
            )
            ...
            if (
                len(potential_symbol) >= 2
                and len(potential_symbol) <= 15
                and not potential_symbol.isdigit()
                and not potential_symbol.startswith("PUBLICATION")
                and not is_repetitive
                and not has_company_suffix
                and potential_symbol
                not in ["NSE", "INR", "RS", "CRORE", "LAKH", "TOTAL"]
            ):
        ...
        if len(words) >= 2:  # At least symbol + company name
            ...
            if (
                len(potential_symbol) >= 2
                and len(potential_symbol) <= 15
                and potential_symbol.isupper()
                and not potential_symbol.isdigit()
                and potential_symbol
                not in ["NSE", "INR", "RS", "CRORE", "LAKH", "TOTAL", "PUBLICATION"]
            ):
    ...
    if text:
        ...
        for line in lines:
            ...
            if words:
                ...
                if (
                    len(first_word) >= 2
                    and len(first_word) <= 15
                    and first_word.isupper()
                    and not first_word.isdigit()
                ):
