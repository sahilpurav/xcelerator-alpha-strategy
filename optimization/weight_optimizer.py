...
class WeightOptimizer:
    ...
    def run_backtest_with_weights(self, weights: Tuple[float, float, float], 
    ...
    def grid_search(self, weight_step: float = 0.1, verbose: bool = True) -> Dict:
        ...
        for w1 in range(0, step + 1):
            for w2 in range(0, step + 1 - w1):
                w3 = step - w1 - w2
                if w3 >= 0:
                    weights.append((w1 * weight_step, w2 * weight_step, w3 * weight_step))
        ...
        if verbose:
            ...
            print(f"🔢 Testing {len(weights)} weight combinations (step: {weight_step})")
            print(f"=" * 80)
        ...
        for i, (w1, w2, w3) in enumerate(weights):
            ...
            if verbose and (i + 1) % 10 == 0:
                ...
                print(f"📊 Progress: {i + 1}/{len(weights)} ({progress_pct:.1f}%) | "
        ...
        if verbose:
            self._print_optimization_results(results_df, best_weights, best_cagr, best_result, valid_count, rejected_count, len(weights))
    ...
    def scipy_optimize(self, verbose: bool = True) -> Dict:
        ...
        try:
            ...
            raise ImportError("scipy is required for this optimization method. Install with: pip install scipy")
        ...
        constraints = [
            {'type': 'eq', 'fun': lambda w: np.sum(w) - 1.0}  # Sum equals 1
        ]
        ...
        if result.success:
            ...
            if verbose:
                ...
                print("=" * 60)
                print(f"🏆 Optimal weights: Return={best_weights[0]:.3f}, "
    ...
    def compare_combinations(self, combinations: List[Tuple[float, float, float]]) -> pd.DataFrame:
        """
        ...
        print(f"=" * 80)
        print(f"{'#':<3} {'Weights':<20} {'CAGR%':<8} {'MaxDD%':<8} {'Sharpe':<7} {'Status':<15}")
        print(f"=" * 80)
        ...
        for i, weights in enumerate(combinations):
            ...
            if result:
                ...
                print(f"{i+1:<3} {weights_str:<20} {cagr:<8.2f} {max_dd:<8.2f} {sharpe:<7.2f} {status:<15}")
                ...
                weights_str = f"({weights[0]:.1f},{weights[1]:.1f},{weights[2]:.1f})"
                print(f"{i+1:<3} {weights_str:<20} {'N/A':<8} {'N/A':<8} {'N/A':<7} {'❌ Failed':<15}")
        
        results_df = pd.DataFrame(results)
        
        if not results_df.empty:
            # Sort by CAGR (descending) for valid combinations first
            results_df = results_df.sort_values(['meets_constraint', 'cagr'], ascending=[False, False])
            ...
            if valid_count > 0:
                ...
                print(f"🏆 Best combination: ({best['return_weight']:.1f}, {best['rsi_weight']:.1f}, {best['proximity_weight']:.1f})")
                print(f"   CAGR: {best['cagr']:.2f}% | Max DD: {best['max_drawdown']:.2f}% | Sharpe: {best['sharpe_ratio']:.2f}")
        
        return results_df
    
    def _print_optimization_results(self, results_df, best_weights, best_cagr, best_result, valid_count, rejected_count, total_combinations):
        ...
        if best_weights:
            ...
            current_result = self.run_backtest_with_weights((0.8, 0.1, 0.1), verbose=False)
            if current_result:
            ...
            print(f"   All tested combinations exceeded the max drawdown threshold of {self.max_drawdown_threshold}%")
            print(f"\n💡 SUGGESTIONS:")
