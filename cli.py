...
from execution.optimization import run_compare_weights, run_optimize_weights
...
def withdraw(
    ...
    percent: Optional[float] = typer.Option(
        None, help="Percentage to withdraw (1-100)"
    ),
...
def optimize_weights(
    ...
    step: float = typer.Option(
        0.1, help="Grid search step size (0.05 for fine, 0.1 for coarse, 0.2 for quick)"
    ),
    max_dd: float = typer.Option(
        -20.0, help="Maximum allowed drawdown percentage (e.g., -20.0 for -20%)"
    ),
...
def compare_weights(
    weights: List[str] = typer.Argument(
        ..., help="Weight combinations to compare (e.g., '0.8,0.1,0.1' '0.6,0.2,0.2')"
    ),
    start: str = typer.Option(..., help="Start date (YYYY-MM-DD)"),
    end: Optional[str] = typer.Option(
        None, help="End date (YYYY-MM-DD). Defaults to today."
...
def rank(
    ...
    weights: str = typer.Option(
        "0.8,0.1,0.1",
        help="Ranking weights as 'return,rsi,proximity' (must sum to 1.0)",
    ),
    top_n: int = typer.Option(50, help="Number of top stocks to display"),
    save_results: bool = typer.Option(True, help="Save results to CSV file"),
    force_refresh: bool = typer.Option(
        False, help="Force cache refresh even if data exists"
    ),
):
    """
