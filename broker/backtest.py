...
class BacktestBroker:
    ...
    def __init__(self, initial_capital: float, transaction_cost_pct: float = 0.001190):
        ...
        self.holdings = []  # List of dict: {"symbol": str, "quantity": int, "buy_price": float}
    ...
    def get_portfolio_value(self, price_data: Dict[str, pd.DataFrame], date: pd.Timestamp) -> float:
        """
    ...
    def place_market_order(self, symbol: str, quantity: int, transaction_type: str, 
                          price: float, date: pd.Timestamp) -> Optional[str]:
        """
        ...
        if transaction_type == "BUY":
            ...
            if total_cost > self.cash:
                print(f"❌ Insufficient funds for {symbol}: Need ₹{total_cost:,.2f}, Have ₹{self.cash:,.2f}")
                return None
            ...
            existing_holding = next((h for h in self.holdings if h["symbol"] == symbol), None)
            
            if existing_holding:
                # Update average price
                total_quantity = existing_holding["quantity"] + quantity
                total_value = (existing_holding["quantity"] * existing_holding["buy_price"]) + transaction_value
            ...
            if not holding or holding["quantity"] < quantity:
                print(f"❌ Insufficient shares to sell {symbol}: Need {quantity}, Have {holding['quantity'] if holding else 0}")
                return None
