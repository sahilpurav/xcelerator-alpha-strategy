...
def _allocate_capital_equally_with_cap(
    ...
    sorted_stocks = sorted(updated_stocks, key=lambda x: x['quantity'] * x['last_price'])
    ...
    for stock in sorted_stocks:
        ...
        additional_capital_to_allocate = min(additional_capital_needed, remaining_usable_capital)
...
def _allocate_leftover_iterative_equal_distribution(
    ...
    while remaining_capital > 0:
        ...
        sorted_allocations = sorted(allocations, key=lambda x: x['last_price'] * x['quantity'])
    ...
    updated_allocations = [symbol_to_allocation[original['symbol']] for original in allocations]
...
def _validate_smart_rebalance_inputs(
    ...
    if not isinstance(transaction_cost_pct, (int, float)) or transaction_cost_pct < 0 or transaction_cost_pct >= 1:
        raise ValueError(f"transaction_cost_pct must be between 0 and 1, got: {transaction_cost_pct}")
    
    # 2. At least one category must have data
    if not held_stocks and not new_stocks and not removed_stocks:
        raise ValueError("At least one of held_stocks, new_stocks, or removed_stocks must be non-empty")
    
    # 3. Business logic validation
    if removed_stocks and not new_stocks:
        raise ValueError("If there are removed_stocks, there must be new_stocks to deploy the freed capital")
    
    # 4. Data structure validation for each category
    def validate_stock_list(stocks: list[dict], category_name: str, expected_quantity_rule: str):
        ...
        for i, stock in enumerate(stocks):
            ...
            for field in required_fields:
                if field not in stock:
                    raise ValueError(f"{category_name}[{i}] missing required field: '{field}'")
            
            # Symbol validation
            if not isinstance(stock['symbol'], str) or not stock['symbol'].strip():
                raise ValueError(f"{category_name}[{i}] 'symbol' must be a non-empty string")
            
            # Price validation
            if not isinstance(stock['last_price'], (int, float)) or stock['last_price'] <= 0:
                raise ValueError(f"{category_name}[{i}] 'last_price' must be a positive number")
            
            # Rank validation (can be None for cash equivalents)
            if stock['rank'] is not None and (not isinstance(stock['rank'], int) or stock['rank'] <= 0):
                raise ValueError(f"{category_name}[{i}] 'rank' must be a positive integer or None")
            
            # Quantity validation
            if not isinstance(stock['quantity'], int) or stock['quantity'] < 0:
                raise ValueError(f"{category_name}[{i}] 'quantity' must be a non-negative integer")
            
            # Business rule validation for quantities
            if expected_quantity_rule == "zero" and stock['quantity'] != 0:
                raise ValueError(f"{category_name}[{i}] 'quantity' should be 0 for new stocks to buy")
            elif expected_quantity_rule == "positive" and stock['quantity'] <= 0:
                raise ValueError(f"{category_name}[{i}] 'quantity' should be positive for existing holdings")
...
def plan_rebalance(
    ...
    _validate_smart_rebalance_inputs(held_stocks, new_stocks, removed_stocks, transaction_cost_pct)
    
    # Calculate freed capital from removed stocks
    freed_capital = cash + sum(stock['quantity'] * stock['last_price'] for stock in removed_stocks)

    if freed_capital <= 0:
        print("✅ Nothing to rebalance.")
        return pd.DataFrame(data=[], columns=[
            "Symbol", "Rank", "Action", "Price", "Quantity", "Invested"
        ])

    # Calculate transaction costs
    sell_value = sum(stock['quantity'] * stock['last_price'] for stock in removed_stocks)
    ...
    allocations, leftover_capital = _allocate_leftover_iterative_equal_distribution(
        ...
        leftover_capital=leftover_capital
    )
    ...
    for allocation in allocations:
        if allocation['quantity'] == 0:
            print(f"❌ Insufficient capital: Cannot allocate funds to all {len(allocations)} stocks.")
    ...
    previous_quanitities = {stock['symbol']: stock['quantity'] for stock in all_portfolio_stocks}
