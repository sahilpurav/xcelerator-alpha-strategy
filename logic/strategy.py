...
def run_strategy(
    ...
    if not market_is_strong:
        return (
            ...
            [{"symbol": sym, "action": "SELL", "rank": None} 
             for sym in held_symbols if sym != cash_symbol_clean] +
    ...
    ranked_df_clean["symbol"] = ranked_df_clean["symbol"].str.replace(".NS", "", regex=False)
    ...
    raw_new_entries = [s for s in top_n_symbols if s not in held_stocks_set][:max_new_entries]
